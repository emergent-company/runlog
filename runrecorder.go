...
func NewRunRecorder(db *RunDB) *RunRecorder {
	r := &RunRecorder{
		startedAt: time.Now(),
		db:        db,
	}
...
func (r *RunRecorder) RegisterRun(testName string) (string, error) {
	r.runID = ""
	r.active = false
	r.seq = 0
...
func (r *RunRecorder) MarkDone(runID string, passed bool) error {
	if !r.active {
		return nil
	}
...
func (r *RunRecorder) Event(kind, message string, details map[string]any) error {
...
//	    return cmd.Run()
//	})
func (r *RunRecorder) CLICapture(description string, fn func() error) string {
	oldOut := os.Stdout
	oldErr := os.Stderr
	rOut, wOut, _ := os.Pipe()
	rErr, wErr, _ := os.Pipe()
...
func (r *RunRecorder) HTTPCall(method, url string, statusCode int, requestBody, responseBody string) {
...
func (r *RunRecorder) emit(kind, message string, details map[string]any) error {
	if !r.active {
		return nil
	}
	r.seq++
...
func ExtractOutput(output, prefix string) string {
